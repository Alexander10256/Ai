...
- `--fetch-backoff` — базовый множитель паузы между повторными попытками.
...
- `history/` — архив всех срезов. Старые файлы автоматически удаляются при превышении лимита (по умолчанию 100 файлов).

Записи ведутся атомарно, что защищает историю от повреждений при перезапуске процесса.

## Как работает фильтрация

1. Каждая новость разбивается на ключевые слова, удаляются часто встречающиеся стоп-слова и нормализуются формы слов (множественное число, суффиксы, окончание `-ing`).
...
- Для каждого RSS-запроса можно задать таймаут и количество повторных попыток (см. `SourceConfig`).
- При временных сетевых ошибках источник автоматически повторяет попытки с экспоненциальной паузой.
- В JSON с дополнительными источниками поддерживаются поля `timeout`, `max_retries` и `retry_backoff`.
...
- Запустите тесты `pytest` перед выкладкой изменений, чтобы убедиться в корректности анализа и хранения.
