...
- Парсинг HTML-страниц с видео и сбор активности (просмотры/лайки/комментарии) для быстрого реагирования на всплески интереса.
...
[
  ...
  {"name": "New Launch Video", "url": "https://example.com/watch?v=42", "kind": "video", "language": "en"}
]
```
...
### Видео-страницы и активность пользователей

- Укажите `"kind": "video"` в описании источника, чтобы монитор подтягивал код страницы видео и извлекал ключевые метрики активности.
- Отпечаток формируется по URL и счётчикам просмотров/лайков/комментариев, поэтому новая волна интереса автоматически попадёт в выдачу трендов.
- Summary события включает автора, время загрузки, первые теги и краткое описание, помогая быстро понять контекст ролика.

## Расширение

- Добавляйте собственные RSS-источники через JSON-конфигурацию, указывая язык и страну для лучшей нормализации.
- Добавляйте HTML-страницы видео (`"kind": "video"`), чтобы отслеживать всплески просмотров, лайков и комментариев конкретных роликов.
